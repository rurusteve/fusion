...
class TextDataLoader:
    ...
    def parseData(self, json, audio_features, video_features_file=None, context_video_features_file=None, text_bert_embeddings=None, context_bert_embeddings=None):
        ...
        for idx, ID in enumerate(json.keys()):
            self.data_input.append((json[ID]["utterance"], json[ID]["speaker"], json[ID]["context"],
                                    json[ID]["context_speakers"], text_bert_embeddings[idx] if text_bert_embeddings else None,
    ...
    def StratifiedKFold(self, splits=5):
        ...
        split_indices = [(text_train_index, text_test_index) for text_train_index, text_test_index in skf.split(self.data_input, self.data_output)]
    ...
    def fullDatasetVocab(self):
        ...
        for utterance in utterances:
            clean_utt = TextDataHelper.clean_str(utterance)
        ...
        for context in contexts:
            for c_utt in context:
                clean_utt = TextDataHelper.clean_str(c_utt)
                utt_words = nltk.word_tokenize(clean_utt)
                for word in utt_words:
                    vocab[word.lower()] += 1
        return vocab
...
class TextDataHelper:
    ...
    def __init__(self, text_train_input, text_train_output, text_test_input, text_test_output, config, dataLoader):
        self.dataLoader = dataLoader
        self.config = config
        self.text_train_input = text_train_input
        self.text_train_output = text_train_output
        self.text_test_input = text_test_input
        self.text_test_output = text_test_output
    ...
    def getData(self, ID=None, mode=None, error_message=None):
        ...
        if mode == "train":
            return [instance[ID] for instance in self.text_train_input]
        elif mode == "test":
            return [instance[ID] for instance in self.text_test_input]
        else:
            print(error_message)
            exit()
    ...
    def oneHotOutput(self, mode=None, size=None):
        ...
        '''
        if mode == "train":
            labels = self.toOneHot(self.text_train_output, size)
        elif mode == "test":
            labels = self.toOneHot(self.text_test_output, size)
        else:
...
if __name__ == "__main__":
    dataLoader = TextDataLoader(config.Config())